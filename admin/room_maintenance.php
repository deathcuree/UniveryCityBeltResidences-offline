    ...
    <div class="row">
      <div class="col-lg-12">
        <div class="card mt-3">
          <div class="card">
            <div class="card-body">
              <h1>Maintenance Request Lists</h1>
...
<script type="text/javascript">
  var selectedData;
  var getAllMaintenance = ajaxPostLink({"action":"getAllMaintenance"});
  ...
  $('#tableContainer').on('click', 'tbody tr', function () {
    ...
    if (selectedData!=undefined&&selectedData.isApprove==0) {
      ...
      Swal.fire({
        ...
        denyButtonText: 'Ongoing',
      }).then((result) => {
        if (result.isConfirmed) {

          var res = ajaxPostLink({"action":"approveRequest","table":"room_maintainance_tbl","id":selectedData.id});
          console.log(res);
          ...
          Swal.fire(
            ...
            getAllMaintenance = ajaxPostLink({"action":"getAllMaintenance"});
            loadDatatable(getAllMaintenance);
          });

        }else{
            var res = ajaxPostLink({"action":"disApproveRequest","table":"room_maintainance_tbl","id":selectedData.id});
            console.log(res);
  ...
  function loadDatatable(dataSent){
    ...
    $('#tableContainer').DataTable({
      ...
      "createdRow": function( row, data, dataIndex){
        if (data['isApprove'] == "0") {
          $(row).find("td:eq(4)").text("Pending").addClass("text-warning")
        }else if (data['isApprove'] == "1"){
          $(row).find("td:eq(4)").text("Completed").addClass("text-success")
        }else if (data['isApprove'] == "2"){
          $(row).find("td:eq(4)").text("Completed").addClass("text-primary")
        }else if (data['isApprove'] == "3"){
          $(row).find("td:eq(4)").text("Ongoing").addClass("text-danger")
        }
      },order: [[0,"desc"]]
    });
  }
