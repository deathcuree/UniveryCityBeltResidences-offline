              ...
              <div class="col-xl-9 col-xxl-8">
                ...
                <div class="d-flex justify-content-between align-items-center border-bottom px-4 pt-4 flex-wrap">
                  <div class="d-flex align-items-center pb-3">
                    <div class="fillow-design">
                      <a href="javascript:void(0);"><img src="./images/white_logo.png" alt="logo" width="45" height="45" viewbox="0 0 45 45" fill="none" /></a>
                    </div>
                    <div class="ms-3">
                      <h4 class="fs-20 font-w700">
                        UniverCity Belt Residences
                      </h4>
                      <span>We share about daily life as designer in the
                        world</span>
                    </div>
                  </div>
                ...
                <div class="card-footer border-0 type-massage">
                  ...
                  <div class="input-group">
                    <textarea class="form-control" id="msg_here" rows="3" placeholder="type message here..."></textarea>
                  </div>
                  <div class="input-group-append d-flex justify-content-between flex-wrap">
                    <div></div>

                    <div>
                      <button type="button" class="btn btn-primary rounded text-white" id="sendBtn">
...
<script type="text/javascript">
  var getAllChatLogs = ajaxPostLink({"action":"getAdminChatLogs","id":userSession.id});
      ...
      '<div class="chat-bx d-flex border-bottom" data-id="'+getAllChatLogs[x].id+'" id="chatSidebar_'+x+'">'+
        ...
        '<div class="w-100">'+
          '<div class="d-flex mb-1 align-items-center">'+
            '<span class="fs-16 text-black">'+getAllChatLogs[x].userFullname+'</span>'+
          '</div>'+
  ...
  $(".chat-bx").on("click", function(){
    ...
    var msgLogs = ajaxPostLink({"action":"getChatMsgs","sentBy":"user","historyID":$(this).attr("data-id")});
    currentChatHistoryID = $(this).attr("data-id");

    for (var i = 0; i < msgLogs.length; i++) {
      if (msgLogs[i].sentBy == "admin") {
        $("#chartBox").append(
    ...
    var msgchecker = setInterval(function () {
      var msgLogs = ajaxPostLink({"action":"getChatMsgs","sentBy":"user","historyID":currentChatHistoryID});

      for (var i = 0; i < msgLogs.length; i++) {
        if (msgLogs[i].isViewedUser == 0) {
          if (msgLogs[i].sentBy == "admin") {
            $("#chartBox").append(
              '<div class="media my-4 justify-content-start align-items-start">'+
                '<div class="message-received">'+
