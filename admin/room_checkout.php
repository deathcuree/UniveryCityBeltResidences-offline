...
<script type="text/javascript">
  ...
  var selectedData;
  var getAllMaintenance = ajaxPostLink({"action":"getAllCheckout"});
    ...
    if (selectedData!=undefined&&selectedData.isApprove==0) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          var res = ajaxPostLink({"action":"approveRequest","table":"room_checkout_tbl","id":selectedData.id});
          console.log(res);
          ...
          Swal.fire(
            ...
            getAllMaintenance = ajaxPostLink({"action":"getAllCheckout"});
            loadDatatable(getAllMaintenance);
          });

        }else{
            var res = ajaxPostLink({"action":"disApproveRequest","table":"room_checkout_tbl","id":selectedData.id});
            console.log(res);
  ...
  function loadDatatable(dataSent){
    ...
    $('#tableContainer').DataTable({
      ...
      "createdRow": function( row, data, dataIndex){
        if (data['isApprove'] == "0") {
          $(row).find("td:eq(7)").text("For Approval").addClass("text-warning")
        }else if (data['isApprove'] == "1"){
          $(row).find("td:eq(7)").text("Approved").addClass("text-success")
        }else if (data['isApprove'] == "2"){
          $(row).find("td:eq(7)").text("Completed").addClass("text-primary")
        }else if (data['isApprove'] == "3"){
          $(row).find("td:eq(7)").text("Disapproved").addClass("text-danger")
        }
      },order: [[0,"desc"]]
    });
  }
