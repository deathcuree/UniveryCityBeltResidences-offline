var baseURL = 'http://localhost/UniveryCityBeltResidences/'; //for local
// var baseURL = 'http://univercitybeltresidences.online/'; //for deployment
...
function ajaxPostLink(data) {
  var retVals;

  jQuery.ajax({
    url: baseURL + 'model/main.php',
    data: data,
    type: 'POST',
    async: false,
    success: function (response) {
...
function backendHandleFormData(formData) {
  var retVals = '';
...
function ajaxPostLinkNoParse(url, data) {
  var retVals;
...
getBalance = function (networkName, tokenName, smartAddress) {
  ...
  if (networkName == 'trx' || networkName == 'trc20') {
    if (tokenName.toUpperCase() === 'trx'.toUpperCase()) {
      ...
      balanceInnerFunction = ajaxShortLink(
        'test-platform/getTokenBalanceBySmartAddress',
        {
          // 'trc20Address':currentUser['trc20_wallet'],
    ...
    $('#warningReported').html(
      "<b>Important Note:</b><br>TRC20 token transfer may consume energy, if energy is insufficient, TRX will be burned. Please ensure you have more than enough TRX to avoid transfer failure.<br><br> You may check TRC20 TRX Fee at <a href='https://tronstation.io/calculator' target='_blank'>Tronstation.io</a>"
    );
  } else if (networkName == 'bsc') {
    if (tokenName.toUpperCase() === 'bnb'.toUpperCase()) {
      balanceInnerFunction = ajaxShortLink(
        'test-platform/getBinancecoinBalance',
        {
    ...
    $('#warningReported').html(
      "<b>Important Note:</b><br>BSC token transfer will consume transaction fee, if BNB is insufficient the transaction will fail Please ensure you have more than enough BNB to avoid transfer failure.<br><br> <b>Estimated Transaction Fee: </b><span class='text-warning' id='transactionFee'>" +
        estimateGasBsc(
          21000,
          ajaxShortLink('userWallet/getBscGasPrice').gasprice
        ).toFixed(6) +
        ' BNB</span>'
    );
    ...
    $('#warningReported').html(
      "<b>Important Note:</b><br>ERC20 token transfer will consume transaction fee, if ETH is insufficient the transaction will fail Please ensure you have more than enough ETH to avoid transfer failure.<br><br> <b>Estimated Transaction Fee: </b><span class='text-warning' id='transactionFee'>" +
        estimateGasEth(
          21000,
          ajaxShortLink('userWallet/getEthGasPrice').gasprice
        ).toFixed(6) +
        ' ETH</span>'
    );
  }
...
getDaysDate = function (numDays) {
  ...
  for (var i = numDays; i >= 0; i--) {
    ...
    dateContainer.push(innerDate.getMonth() + 1 + '-' + innerDate.getDate());
  }
...
getDaysDateV2 = function (numDays) {
  ...
  for (var i = numDays; i >= 0; i--) {
    ...
    dateContainer.push(year + '-' + month + '-' + day);
  }
